...
// extern variable
extern BNP ToolsLLHeadP;
extern UInt16 BlockPointerState;// = HANDBitmap; // block : HAND , THREAD


Boolean ToolsFormHandleEvent(EventPtr eventP)
{
	...
	switch (eventP->eType) 
		...
		case frmOpenEvent:
			...
			break;
		case penDownEvent:
			InAreaInf = IsInArea(InAreaInf,ToolsLLHeadP,eventP->screenX,eventP->screenY,0,0); //LLP will set
			if(InAreaInf.InArea)
			{
